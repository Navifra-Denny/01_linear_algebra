...
points_T = points.T
print(f"type of points_T: {type(points_T)}")
print(f"shape of points_T: {points_T.shape}")
print("# data form d x n, where d = feature dimension, n = number of samples")

# 평균 계산: 각 feature (각 행)에 대한 평균 (결과 shape: (d, 1))
mean_T = np.mean(points_T, axis=1, keepdims=True)  # shape: (3, 1)

# 중심화된 데이터 centered_points (d × n): 각 열에서 평균을 뺌
centered_points = points_T - mean_T  # shape: (3, n)

print("")
print("Point cloud 평균:", mean_T)
print(f"mean_T shape: {mean_T.shape}")
print(f"centered_points shape (centered data): {centered_points.shape}")


# 중심화된 포인트 클라우드 생성
centered_pcd = o3d.geometry.PointCloud()
centered_pcd.points = o3d.utility.Vector3dVector(centered_points.T)

# 색상 지정 (회색)
gray = np.tile([0.5, 0.5, 0.5], (centered_points.shape[1], 1))
...
print("")
print("----------------------------------------------------------")
print("3. covariance matrix calculated by B•B^T")
B = centered_points  # B: (d × n) matrix

print("3.1 calculating the covariance without library. covariance matrix calculated by B•B^T")

# 공분산 행렬 계산: (1 / (n - 1)) * B * B^T
n_samples = B.shape[1]
cov_matrix = (B @ B.T) / (n_samples - 1)
...
# 공분산 행렬 계산 (X: d × n → 공분산: d × d)
cov_matrix_w_lib = np.cov(centered_points, rowvar=True)
print("Covariance matrix (3x3):\n", cov_matrix_w_lib)
...
print("5.1. 모든 주성분 사용")
# 모든 주성분 사용
Z = eigenvectors.T @ B  # 회전된 좌표계 (3 x n)

# PCA 좌표계에 투영된 포인트 클라우드 생성
pca_aligned_pcd = o3d.geometry.PointCloud()
pca_aligned_pcd.points = o3d.utility.Vector3dVector(Z.T)

# 색상 지정 (녹색)
green = np.tile([0.0, 1.0, 0.0], (Z.shape[1], 1))
pca_aligned_pcd.colors = o3d.utility.Vector3dVector(green)

o3d.visualization.draw_geometries([centered_pcd, pca_aligned_pcd, coordinate_frame, line_set])
...
# 2D PCA 좌표계로 투영
Z2D = V2.T @ B   # shape: (2, n)

# 2D → 3D로 확장 (z축에 0 추가)
Z2D_3D = np.vstack((Z2D, np.zeros((1, Z2D.shape[1]))))  # shape: (3, n)

# 포인트 클라우드 생성
pca_2D_pcd = o3d.geometry.PointCloud()
pca_2D_pcd.points = o3d.utility.Vector3dVector(Z2D_3D.T)

# 색상 지정 (파란색)
blue = np.tile([0.0, 0.0, 1.0], (Z2D.shape[1], 1))
...
# 1. 역회전: Z2D (2xn) → B 복원 (3xn)
B_hat = V2 @ Z2D   # shape: (3, n)

# 2. 평균 복원
X_hat = B_hat + mean_T  # shape: (3, n)

# 복원된 포인트 클라우드 생성
reconstructed_pcd = o3d.geometry.PointCloud()
reconstructed_pcd.points = o3d.utility.Vector3dVector(X_hat.T)

# 색상 지정 (보라색)
purple = np.tile([0.6, 0.0, 0.6], (X_hat.shape[1], 1))
